...
export async function sendResetCode(email: string, code: string, companyName?: string) {
  const fromName = process.env.SMTP_FROM_NAME || 'SupplyHub';
  const fromEmail = process.env.SMTP_FROM || process.env.SMTP_USER || 'noreply@supplyhub.com';

  await transporter.sendMail({
    from: `"${fromName}" <${fromEmail}>`,
    to: email,
    subject: 'Password Reset Code - SupplyHub',
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <div style="text-align: center; margin-bottom: 24px;">
          <h1 style="font-size: 24px; color: #1e293b; margin: 0;">SupplyHub</h1>

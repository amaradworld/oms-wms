...
export async function sendOrderConfirmation(email: string, orderNumber: string, customerName: string, items: string, source: string) {
  await transporter.sendMail({
    from: getFromAddress(),
    to: email,
    ...
    html: emailWrapper(`
      <div style="background: #f0fdf4; border-radius: 12px; padding: 24px; border: 1px solid #bbf7d0;">
        ...
        <p style="color: #475569; margin: 0 0 16px; font-size: 14px;">Hi ${customerName}, your order <strong>#${orderNumber}</strong> has been confirmed via ${source}.</p>
        <p style="color: #475569; margin: 0 0 16px; font-size: 13px;">Items: ${items}</p>
...
export async function sendDispatchNotification(email: string, orderNumber: string, customerName: string, source: string, courier?: string, awbNumber?: string, trackingUrl?: string) {
  await transporter.sendMail({
    from: getFromAddress(),
    to: email,
    ...
    html: emailWrapper(`
      <div style="background: #eff6ff; border-radius: 12px; padding: 24px; border: 1px solid #bfdbfe;">
        ...
        <div style="background: #ffffff; border-radius: 8px; padding: 16px; border: 1px solid #e2e8f0;">
          ${courier ? `<p style="margin: 0 0 8px; font-size: 13px; color: #475569;"><strong>Courier:</strong> ${courier}</p>` : ''}
          ${awbNumber ? `<p style="margin: 0 0 8px; font-size: 13px; color: #475569;"><strong>AWB Number:</strong> ${awbNumber}</p>` : ''}
...
export async function sendDeliveryNotification(email: string, orderNumber: string, customerName: string, source: string) {
  await transporter.sendMail({
    from: getFromAddress(),
    to: email,
    ...
    html: emailWrapper(`
      <div style="background: #f0fdf4; border-radius: 12px; padding: 24px; border: 1px solid #bbf7d0;">
        ...
        <p style="color: #475569; margin: 0 0 16px; font-size: 14px;">Hi ${customerName}, your order <strong>#${orderNumber}</strong> has been delivered successfully.</p>
